...
namespace tsenaFinal.models
{
    internal class Paiement
    {
        ...
        public static decimal SommeTokonyNaloanyByDate(Connexion connexion, int idPerson, int mois, int annee)
        {
            ...
            //int moisTRef = Math.Min(maxDateT, moisT);
            int moisTRef = moisT; // min(maxDateT, moisT) dans Python, ici juste moisT
            ...
            foreach (var contrat in allContrat)
            {
                ...
                while (moisDebT <= moisTRef)
                {
                    ...
                    // Console.WriteLine($"Reste pour idBox={contrat.IdBox}, mois={moisDeb}, année={anneeDeb} : {reste}");
                    totalDettes += reste;
            ...
            Console.WriteLine($"Somme total tokony aloany pour idPerson={idPerson} jusqu'à {mois}/{annee} : {totalDettes}");
            return totalDettes;
        }

        public static decimal SommeDetteTokonyNaloany(Connexion connexion, int idPerson, int mois, int annee)
        {
            decimal efaVoaloa = sumNaloanyPersonByDate(connexion, idPerson, mois, annee);
            decimal tokonyAloa = SommeTokonyNaloanyByDate(connexion, idPerson, mois, annee);
